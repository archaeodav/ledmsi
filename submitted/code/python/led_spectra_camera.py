...
def spectra(file):
    '''
    Function loads spectra from file
    ...
    averaged : TYPE ndarray
        DESCRIPTION. averaged obesrvations
    norm : TYPE ndarray
        DESCRIPTION. normlsied spectra
...
def get_fwhm(data,
             ht = 0.5):
    '''
    Function gets full width at half maxima for LED emisison spectra

    Parameters
    ----------
    data : TYPE ndarray
        DESCRIPTION.input spectra
    ht : TYPE, float
        DESCRIPTION. height relative to maxima to measure the width of peaks
        default is 0.5.
...
def plot(wl,
    ...
    # Show the plot
    #plt.show()
    plt.savefig(root_dir / "output" / "led_spectra.png", dpi=300)
...
def rgb_plot(file=None):
    ...
    #plt.show()
    plt.savefig(root_dir / "output" / "camera_sensitivity.png", dpi=300)
